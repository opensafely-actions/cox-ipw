...
option_list <- list(
  make_option(
    ...
    type = "character",
    default = "input.csv",
    help = "Input dataset. csv, csv.gz, rds, arrow, or a feather filename (this is assumed to be within the output directory but can be within a subdirectory) [default %default]",
  ...
  make_option(
    ...
    help = "Semi-colon separated list of other covariates or filename of text file containing semi-colon separated list of other covariates to be included in the regression model; specify argument as NULL to run age, age squared, sex adjusted model only [default %default]",
    metavar = "varname_1;varname_2;... or covariate-other.txt"
  ),
  make_option(
    "--cox_start",
    type = "character",
    default = "pat_index_date",
  ...
  make_option(
    ...
    type = "character",
    default = "results.csv",
    help = "Output data csv filename (this is assumed to be within the output directory but can be within a subdirectory) [default %default]",
    metavar = "filename.csv"
  ),
  make_option(
    "--seed",
...
print(paste0(
  "Exposure data range: ",
  min(input$exposure, na.rm = TRUE),
  " to ",
  max(input$exposure, na.rm = TRUE)
))
print(paste0(
  "Outcome data range: ",
  min(input$outcome, na.rm = TRUE),
  " to ",
...
input$outcome_status <- input$outcome == input$fup_stop &
  !is.na(input$outcome) &
  !is.na(input$fup_stop)
...
if (length(cut_points) == 1L) {
  ...
  time_period_labels <- paste0(
    "days",
    c("0", cut_points[1:(length(cut_points) - 1)]),
    "_",
    cut_points
  )
}
...
if (
  ...
  if (!is.null(covariate_other)) {
    ...
    print(
      "Additional covariates specified: Remove covariates with insufficient variation"
    )

    tmp <- check_covariates(
      df = data_surv,
  ...
  if (opt$run_analysis == TRUE) {
    ...
    tmp <- data.frame(
      ...
      covariate_collapsed = "",
      obs_warning = "",
      N_events = episode_info[
        episode_info$time_period == "days_pre",
      ]$N_events,
      person_time_total = episode_info[
        episode_info$time_period == "days_pre",
      ]$person_time_total,
      outcome_time_median = episode_info[
        episode_info$time_period == "days_pre",
      ]$outcome_time_median,
      stringsAsFactors = FALSE
    )
...
write.csv(results, file = paste0("output/", opt$df_output), row.names = FALSE)
