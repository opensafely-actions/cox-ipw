...
if(prior_history_var != ""){
  read_in_cols <- unique(append(read_in_cols, c(prior_history_var, covar_vars)))
}else{
  read_in_cols <- unique(append(read_in_cols, c(covar_vars)))
}

input <- readr::read_csv("output/input.csv")
input <- input %>% dplyr::select(dplyr::all_of(read_in_cols))
...
cuts_days_since_expo <- c(28, 197) # cuts_days_since_expo <- c(14, 28, 56, 84, 197) 
...
if(cohort=="vaccinated"){
  input <- input %>% 
    dplyr::rowwise() %>% 
    dplyr::mutate(follow_up_end=min(event_date, DATE_OF_DEATH,cohort_end_date,na.rm = TRUE))
}else if(cohort=="electively_unvaccinated"){
  input <- input %>% 
    dplyr::rowwise() %>% 
    dplyr::mutate(follow_up_end=min(vax_date_covid_1,event_date, DATE_OF_DEATH,cohort_end_date,na.rm = TRUE))
}
